...
- `Modules/Sources/Adapters/APIClient/APIClientLive.swift` is the live anti-corruption layer. It is responsible for calling the generated `APIProtocol`, handling `.ok` / `.internalServerError` / `.undocumented`, and coordinating auth, cache, and session side effects.
- `Modules/Sources/Adapters/APIClient/APIClientMappers.swift` owns handwritten translation between generated OpenAPI schema types and Domain models.
