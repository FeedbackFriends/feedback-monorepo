...
/**
 * Table for storing feedback questions for activities and session snapshots.
 *
 * Each row represents either:
 * - a canonical question owned by an activity, or
 * - a question snapshot copied into a session at creation time.
 *
 * Relationships:
 * - References [AccountTable] (manager), optionally [ActivityTable], and optionally [SessionTable].
 * - Deleting a manager, activity, or session cascades and removes related questions.
 *
 * Columns:
 ...
 * @property session Optional foreign key to [SessionTable.id].
 * @property activityQuestionId Stable canonical identifier copied from the activity-level question.
 * @property index The order of the question in the owning activity/session.
 */
object QuestionTable: CommonColumnsTbl("question") {
    ...
    val session = optReference(name = "session_id", SessionTable, onDelete = ReferenceOption.CASCADE)
    val event = session
